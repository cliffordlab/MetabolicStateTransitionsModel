...
%   Λ(r,t) = λ(t) - b r^2
%   Ω(r,t) = ω0 + a r^2
% with λ(t) chosen so that the limit-cycle radius grows smoothly over time.
...
% Phase-plane domain
xy_lim      = 2;    % both x and y in [-xy_lim, xy_lim]
...
%  MOVIE SETTINGS
%  ==============================
make_movie      = true;                           % set false if you just want on-screen animation
movie_filename  = 'Movie/case_2_movie.mp4';
movie_fps       = 10;                             % frames per second in the video

%% ==============================
%  ORDER-2 LAMBDA–OMEGA PARAMETERS (GROWING LIMIT CYCLE)
%  ==============================
omega0 = 1.0;   % base frequency term
a      = 1.0;   % quadratic freq term
b      = 1.0;   % quadratic amp term

% Time-varying limit-cycle radius r̄(t) that grows smoothly
R_min    = 0.6;   % initial small limit-cycle radius
R_max    = 1.0;   % maximum radius as t -> large
tau_grow = 10.0;  % time constant for exponential growth

% Desired LC radius (growing from R_min to R_max)
R_target = @(t) R_max - (R_max - R_min) * exp(-t / tau_grow);

% Time-varying lambda(t) chosen so that LC radius ~ R_target(t)
lambda_t = @(t) b * (R_target(t)).^2;   % λ(t) = b r̄(t)^2
...
%% ==============================
%  NULLCLINE SNAPSHOTS EVERY 10 CYCLES (APPROX)
%  ==============================
Omega_init        = Omega_fn(R_min, 0);
...
%  SOLVE ODE
%  ==============================
odefun = @(t, x) lambda_omega_rhs_order2_growingLC(t, x, Lambda_fn, Omega_fn);
...
% Precompute max cumulative value for axis scaling (cumulative |diff| of peaks)
if numel(x_peaks) >= 2
...
%% ==============================
%  ADAPTATION WINDOW (FOR SHADING) – based on |Δpeak|
%  (safe bounds check; avoids indexing errors)
%  ==============================
diffThreshold = 0.1;
leverage      = 4;
...
contour(ax1, Xg2, Yg2, Ux0, [0 0], ...
    'Color', xnull_color, 'LineWidth', lineWidthMain, 'HandleVisibility','off');
contour(ax1, Xg2, Yg2, Vy0, [0 0], ...
    'Color', ynull_color, 'LineWidth', lineWidthMain, 'HandleVisibility','off');

% Trajectory (for legend)
hTrajLeft = plot(ax1, x_traj(1), y_traj(1), ...
    'Color', traj_color, 'LineWidth', lineWidthTrajectory, ...
    'DisplayName','Trajectory');

% Moving point on trajectory (no legend)
movingDotSize = 10;
...
% Peaks in phase plane (no legend)
...
for k = 2:frame_skip:N
    ...
    % Update trajectory + moving point
...
%  LOCAL FUNCTION
%  ==============================
function dxdt = lambda_omega_rhs_order2_growingLC(t, x, Lambda_fn, Omega_fn)
    r   = hypot(x(1), x(2));
    lam = Lambda_fn(r, t);
    om  = Omega_fn(r, t);
